...
finalRun = 'no'
numberOfRuns = 1
userMaterial = 'titanium alloy'             # 'aluminium', 'silicon carbide', 'titanium alloy'
nnx = 15                                    # number of Unit cells in X direction
fac = 0.0                                   # distortion
CoarseElemSize = unitCellSize/5.0           # minimum coarse element size
FineElemSize = unitCellSize/20.0            # mimimum fine element size
...
if latticeType.lower() == "fcc":            # 0.02, 1
    strainApp = 0.02                        # amplitude (uniax = strainApp * H; FT = ct_ut * stainApp * H)
    ct_ut = 1
elif latticeType.lower() == "tri":          # 0.02, 1
    strainApp = 0.02
    ct_ut = 1
elif latticeType.lower() == "kagome":       # 0.02, 1
    strainApp = 0.02
    ct_ut = 1
elif latticeType.lower() == "hex":          # 0.03, 1
    strainApp = 0.03
    ct_ut = 1

os.chdir("C:\\Users\\exy053\\Documents\\ModelChanges") #PerConv2\\" + str(unitCellSize) + "\\" + latticeType) # + "\\" + str(relDensity))

############################################################################################
####################################### FUNCTIONS ##########################################
############################################################################################
	
def node_FCC(L, H, nnx, nny, fac, distribution):
...
def node_Tri(L, H, nnx, nny, unitCellSize, fac, distribution):
...
def node_Kagome(L, H, nnx, nny, unitCellSize, fac, distribution):
    ...
    totalNodes = int((2*nnx*(nny+1)) + (nnx-1)*math.ceil(nny/2.0) + (nnx)*floor(nny/2))
    nodes = zeros(shape=(totalNodes,3))
    nodesR = zeros(shape=(totalNodes,3))
...
if (fac == 0.0 and sizeVar == 'no'):
    imper = 'per'
elif (fac != 0.0 and sizeVar == 'no'):
    imper = 'disNodes'
elif (fac == 0.0 and sizeVar == 'yes'):
    imper = 'disStruts'
else:
    ...
    if (latticeType.lower() == 'fcc'):
        ...
        if round(nny) % 2.0 == 0:
            if H/L >= 0.96:
                ...
                nny = H/unitCellSize
        nny = int(round(nny))
        nodes, nodesR = node_FCC(L, H, nnx, nny, fac, distribution)
        ...
        if round(nny) % 2.0 == 0:
            if H/L >= 0.96:
                ...
                nny = H/unitCellSize
        nny = int(round(nny))
        nodes, nodesR = node_Tri(L, H, nnx, nny, unitCellSize, fac, distribution)
    ...
    if  (MechanicalModel.lower() == 'ductile' or MechanicalModel.lower() == 'both'):
        ...
        if (userMaterial.lower() == 'aluminium'):
            ...
            mdb.models[ModelName].materials[userMaterial].ductileDamageInitiation.DamageEvolution(
                ...
                (0.0, 0.0),
                (0.0036, FineElemSize*0.0021),
                (0.0045, FineElemSize*0.00422), 
                (0.0104, FineElemSize*0.0063),
                (0.022,  FineElemSize*0.0084),
                (0.0336, FineElemSize*0.0105),
                (0.0466, FineElemSize*0.0126),
                (0.0599, FineElemSize*0.0147),
                (0.0761, FineElemSize*0.0168),
                (0.095,  FineElemSize*0.01891),
                (0.1173, FineElemSize*0.02101),
                (0.1443, FineElemSize*0.02311),
                (0.1761, FineElemSize*0.02521),
                (0.2144, FineElemSize*0.02731),
                (0.2578, FineElemSize*0.02928), 
                (0.3029, FineElemSize*0.03098),
                (0.3457, FineElemSize*0.03236),
                (0.3866, FineElemSize*0.03335),
                (0.4301, FineElemSize*0.034),
                (0.4665, FineElemSize*0.03446),
                (1.0,    FineElemSize*0.03547)))
            ...
            mdb.models[ModelName].materials[userMaterial].ductileDamageInitiation.DamageEvolution(
                type=DISPLACEMENT, softening=TABULAR, table=(
                (0.0, 0.0),
                (0.2, FineElemSize*0.001),
                (0.4, FineElemSize*0.002), 
                (0.6, FineElemSize*0.003),
                (0.8, FineElemSize*0.004),
                (1.0, FineElemSize*0.005)))
        ...
        a.seedPartInstance(regions=partInstances, size=CoarseElemSize, deviationFactor=0.1, 
            minSizeFactor=0.1)
        a = mdb.models[ModelName].rootAssembly
        partInstances =(a.instances['Part-1-1'], )
        ...
        a.setElementType(regions=pickedRegions, elemTypes=(elemType1, ))
        partInstances = (a.instances['Part-1-1'], )
        a.seedEdgeBySize(edges=edges, size=FineElemSize, deviationFactor=0.1, 
        ...
        if latticeType.lower() == 'hex' or latticeType.lower() == 'tri':
            left_nodes = []
            bottom_nodes = []
            right_nodes = []
            top_nodes = []
...
# ######################################################################################################            
    ...
    if (MechanicalModel.lower() == 'fracture' or MechanicalModel.lower() == 'both'):
        ...
        if latticeType.lower() == "fcc":
            ...
            a = [(L-(0.5*(3**0.5)*unitCellSize))/((nnx-1)/2)*i for i in range(nnx+1)]
        elif latticeType.lower() == "kagome":
            a = [2*L*i/(2*nnx-1) + 1.5*unitCellSize for i in range(nnx-1)]
        elif latticeType.lower() == "hex":
            if round(nny) % 4 == 3:
                a = [((3**0.5)/2)*unitCellSize + (L-((3**0.5)*unitCellSize))/(nnx-1)*i for i in range(nnx+1)]
            elif round(nny) % 4 == 1:
                a = [L/nnx*i for i in range(nnx+1)]
        a0 = min(a, key=lambda x:abs(x-(W/2.0)))
        
        xCrS = [-0.1*W, H/2-unitCellSize*0.2]    # crack starting point bottomLeft
        xCrE = [a0 - 0.01*unitCellSize, H/2+unitCellSize*0.2]       # crack end point topRight
...
#        print(len(cracktip_node_labels))      
 #       ln = mesh.MeshNodeArray(cracktip_node_labels)
  #      mdb.models[ModelName].rootAssembly.Set(nodes=ln , name='temp')
        ...
        mdb.models[ModelName].TabularAmplitude(name='Amp-1', timeSpan=STEP, 
            smooth=SOLVER_DEFAULT, data=((0.0, 0.0), (0.1, ct_ut*strainApp*H)))
