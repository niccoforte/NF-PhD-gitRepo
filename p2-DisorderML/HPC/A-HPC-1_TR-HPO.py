...
def parse_args():
    parser = argparse.ArgumentParser(description="HPC GPU HPO run for the UT Transformer stress-strain model.")
    ...
    parser.add_argument("--name", default="TR_full_hOpt")
...
def main():
    ...
    from resources.MLfunc import hOpt_best_summary, hOpt_model
    ...
    print(f"Split sizes: {split_size_summary(dat_tr)}")
    ...
    study = hOpt_model(
        ...
        model_space=tr_model_space,
        loss_space=tr_loss_space,
        train_space=tr_train_space,
        seed=args.seed,
        device=device,
        save=True,
