...
#os.chdir(r"D:\ANN_FractureToughness\ABAQUS\FCC_SizeCheck\9X9")
...
latticeType = 'kagome'
FractureCalculations = 'no'
nnx = 4                      # number of Unit cells in X direction
nny = 4                      # number of Unit cells in Y direction
fac = 0.0                    # distortion
elemSize = 10.0                # minimum element size
relDensity = 0.1              # relative density
strainApp = 0.3
...
ModelName    = latticeType + '_' + str(nnx) + 'X' + str(nny) + '_' + str(int(relDensity*100)) + '%'
Job          = latticeType + '_' + str(nnx) + 'X' + str(nny) + imper
#data 	     = sys.argv[-1]
userMaterial = 'Material-5'
PBC          = 'no'
elemType     = B21
...
if (FractureCalculations.lower() == 'yes'):
...
if (FractureCalculations.lower() == 'no'):
...
elif (userMaterial.lower() == 'material-4'):
    mdb.models[ModelName].Material(name=userMaterial)
...
elif (userMaterial.lower() == 'material-5'):
    mdb.models[ModelName].Material(name=userMaterial)
    mdb.models[ModelName].materials[userMaterial].Density(table=((4.42e-09, ), ))
    mdb.models[ModelName].materials[userMaterial].Elastic(table=((123000.0, 0.3), ))
    mdb.models[ModelName].materials[userMaterial].UserDefinedField()
    mdb.models[ModelName].materials[userMaterial].Plastic(table=
        ((930.0,	0),
        (932.0,	0.002),
...
mdb.Job(name=Job, model=ModelName, description='', type=ANALYSIS, 
    ...
    contactPrint=OFF, historyPrint=OFF, userSubroutine='VUSDFLD.f', scratch='', 
