...
def node_new_tri(L, H, nnx, nny, fac, distribution):

    unitX = 0.5*sqrt(3)*L / nnx
    unitY = H / nny
...
latticeType = 'FCC'
nnx = 12                      # number of Unit cells in X direction
nny = 16                      # number of Unit cells in Y direction
fac = 0.00                    # distortion
...
ModelName    = latticeType + '_' + str(nnx) + 'X' + str(nny) + '_' + imper
Job          = sys.argv[-2]
data 	     = sys.argv[-1]
userMaterial = 'Material-3'
PBC          = 'no'
elemType     = B21
...
if (latticeType.lower() == 'fcc'):
    ...
    H = unitCellSize * nny
    nodes, nodesR = node_new_tri(L, H, nnx, nny, fac, distribution)
...
for ik in range(0,len(element)):
    x1 = nodesR[element[ik, 1]-1, 1]
    x2 = nodesR[element[ik, 2]-1, 1]
    y1 = nodesR[element[ik, 1]-1, 2]
    y2 = nodesR[element[ik, 2]-1, 2]
...
for ik in range(len(element)):
    s.Line(point1=nodesR[element[ik][1]-1][1:3], point2=nodesR[element[ik][2]-1][1:3])
...
for numEdge in range(0,len(element)):
    x1 = nodesR[element[numEdge, 1]-1, 1]
    x2 = nodesR[element[numEdge, 2]-1, 1]
    y1 = nodesR[element[numEdge, 1]-1, 2]
    y2 = nodesR[element[numEdge, 2]-1, 2]
    xMid = (x1+x2)/2.0
    yMid = (y1+y2)/2.0
...
if (userMaterial.lower() == 'material-1'):
    ...
    mdb.models[ModelName].materials[userMaterial].UserMaterial(
        mechanicalConstants=(69300.0, 0.3, 117, 350, 0.28, 32, 500))
    mdb.models[ModelName].materials[userMaterial].Depvar(deleteVar=4, n=9)

elif (userMaterial.lower() == 'material-2'):
    mdb.models[ModelName].Material(name=userMaterial)
    mdb.models[ModelName].materials[userMaterial].Density(table=((2.7e-09, ), ))
    mdb.models[ModelName].materials[userMaterial].Elastic(table=((69300.0, 0.3), ))
    mdb.models[ModelName].materials[userMaterial].Plastic(table=(
        (117.0000,	0.000000000),
        (134.6912,	0.000556404),
        (148.4888,	0.001407304),
        (162.2865,	0.002258203),
        (180.0263,	0.003702218),
        (197.1091,	0.005055713),
        (214.1919,	0.007209208),
        (224.7043,	0.009857514),
        (234.5598,	0.012215299),
        (245.0723,	0.014963603),
        (254.2707,	0.017630870),
        (266.0972,	0.023160214),
        (282.5230,	0.032223189),
        (294.3495,	0.043352532),
        (304.2050,	0.054810317),
        (316.6886,	0.073030179)))
    mdb.models[ModelName].materials[userMaterial].DuctileDamageInitiation(
        table=((0.073030179, 0.33333, 0.01), ))
    mdb.models[ModelName].materials[userMaterial].ductileDamageInitiation.DamageEvolution(
        type=ENERGY, table=((0.38*elemSize, ), ))

elif (userMaterial.lower() == 'material-3'):
    mdb.models[ModelName].Material(name=userMaterial)
    mdb.models[ModelName].materials[userMaterial].Density(table=((2.7e-08/elemSize, ), ))
...
if (PBC.lower() == 'yes'):
    RFid = mdb.models[ModelName].rootAssembly.ReferencePoint(point=(L+50.0, H+50.0, 0.0)).id
    r1 = mdb.models[ModelName].rootAssembly.referencePoints
    refPoints1=(r1[RFid], )
    a.Set(referencePoints=refPoints1, name='m1')

    RFid = mdb.models[ModelName].rootAssembly.ReferencePoint(point=(L+150.0, H+50.0, 0.0)).id
    r1 = mdb.models[ModelName].rootAssembly.referencePoints
    refPoints1=(r1[RFid], )
    a.Set(referencePoints=refPoints1, name='n1')

    RFid = mdb.models[ModelName].rootAssembly.ReferencePoint(point=(L+50.0, H+150.0, 0.0)).id
    r1 = mdb.models[ModelName].rootAssembly.referencePoints
    refPoints1=(r1[RFid], )
    a.Set(referencePoints=refPoints1, name='n2')

#elif (PBC.lower() == 'no'):
#    RFid = mdb.models[ModelName].rootAssembly.ReferencePoint(point=(L/2.0, H+50.0, 0.0)).id
#    r1 = mdb.models[ModelName].rootAssembly.referencePoints
#    refPoints1=(r1[RFid], )
#    a.Set(referencePoints=refPoints1, name='load')
#
#    RFid = mdb.models[ModelName].rootAssembly.ReferencePoint(point=(L/2.0, -50.0, 0.0)).id
#    r1 = mdb.models[ModelName].rootAssembly.referencePoints
#    refPoints1=(r1[RFid], )
...
elif (userMaterial.lower() == 'material-2' or userMaterial.lower() == 'material-3'):
    mdb.models[ModelName].fieldOutputRequests['F-Output-1'].setValues(variables=(
        'S', 'LE', 'UT', 'RF', 'SDEG', 'DMICRT', 'STATUS'), numIntervals=200)
...
if (PBC.lower() == 'yes'):
    mdb.models[ModelName].Equation(name='Constraint-1', terms=((-1.0, 
        'Set-left', 1), (-1.0, 'm1', 1)))
    mdb.models[ModelName].Equation(name='Constraint-2', terms=((1.0, 
        'Set-right', 1), (-1.0, 'm1', 1)))

    mdb.models[ModelName].Equation(name='Constraint-3', terms=((-1.0, 
        'Set-bottom', 2), (-1.0, 'm1', 2)))
    mdb.models[ModelName].Equation(name='Constraint-4', terms=((1.0, 
        'Set-top', 2), (-1.0, 'm1', 2)))
    ...
    mdb.models[ModelName].rootAssembly.engineeringFeatures.TwoPointSpringDashpot(
        name='Springs/Dashpots-1', regionPairs=region, axis=NODAL_LINE, 
        springBehavior=ON, springStiffness=1e-4*SpringStiffness, dashpotBehavior=OFF, 
        dashpotCoefficient=0.0)
        
    a = mdb.models[ModelName].rootAssembly
    rgn1pair1=a.sets['m1']
    rgn2pair1=a.sets['n2']
    region=((rgn1pair1, rgn2pair1), )
    mdb.models[ModelName].rootAssembly.engineeringFeatures.TwoPointSpringDashpot(
        name='Springs/Dashpots-2', regionPairs=region, axis=NODAL_LINE, 
        springBehavior=ON, springStiffness=SpringStiffness, dashpotBehavior=OFF, 
        dashpotCoefficient=0.0)

    region=a.sets['m1']
    mdb.models[ModelName].rootAssembly.engineeringFeatures.PointMassInertia(
        name='Inertia-1', region=region, mass=1e-08, alpha=0.0, composite=0.0)
    region=a.sets['n1']
    mdb.models[ModelName].rootAssembly.engineeringFeatures.PointMassInertia(
        name='Inertia-2', region=region, mass=1e-08, alpha=0.0, composite=0.0)
    region=a.sets['n2']
    mdb.models[ModelName].rootAssembly.engineeringFeatures.PointMassInertia(
        name='Inertia-3', region=region, mass=1e-08, alpha=0.0, composite=0.0)

#elif (PBC.lower() == 'no'):
#    a = mdb.models[ModelName].rootAssembly
#    region1=a.sets['fixity']
#    region2=a.sets['Set-bottom']
...
if (PBC.lower() == 'yes'):
    mdb.models[ModelName].TabularAmplitude(name='Amp-1', timeSpan=STEP, 
        smooth=SOLVER_DEFAULT, data=((0.0, 0.0), (0.1, L)))
    mdb.models[ModelName].TabularAmplitude(name='Amp-2', timeSpan=STEP, 
        smooth=SOLVER_DEFAULT, data=((0.0, 0.0), (0.1, H)))
    ...
    mdb.models[ModelName].DisplacementBC(name='BC-2', createStepName='Step-1', 
        region=region, u1=UNSET, u2=1.0, ur3=UNSET, amplitude='Amp-2', fixed=OFF, 
        distributionType=UNIFORM, fieldName='', localCsys=None)
...
if (userMaterial.lower() == 'material-1'):
    userSubroutine = 'isoHard_vumat_damage_v4.f'
elif (userMaterial.lower() == 'material-2' or userMaterial.lower() == 'material-3'):
    userSubroutine = ''
...
mdb.Job(name=Job, model=ModelName, description='', type=ANALYSIS, 
    ...
    resultsFormat=ODB, parallelizationMethodExplicit=DOMAIN, numDomains=32, 
    activateLoadBalancing=False, multiprocessingMode=DEFAULT, numCpus=32)
